...
public class FLMoveSelector : MonoBehaviour
{
    //Overlays
    ...
    void Update()
    {
        ...
        if (Physics.Raycast(ray, out hit))
        {
            ...
            if (Input.GetMouseButtonDown(0))
            {
                ...
                if (FLGameManager.instance.PieceAtGrid(gridPoint) == null && !FLGameManager.instance.PieceHasAttacked(movingPiece))
                {

                    //game manager moves piece
                    FLGameManager.instance.Move(movingPiece, gridPoint);
                    FLGameManager.instance.CountAction();
                }
                else if (!FLGameManager.instance.PieceHasAttacked(movingPiece))
                {
                    if (FLGameManager.instance.RollToCapture(movingPiece, gridPoint))
                    {
                        //if valid move but piece there capture piece
                        FLGameManager.instance.CapturePieceAt(gridPoint);
    ...
    private void CancelMove()
    {
        ...
        FLGameManager.instance.DeselectPiece(movingPiece);
        //get tileSelector and turn it on
        FLTileSelector selector = GetComponent<FLTileSelector>();
        selector.EnterState();
    }
    ...
    public void EnterState(GameObject piece)
    {
        ...
        moveLocations = FLGameManager.instance.MovesForPiece(movingPiece);
        ...
        foreach (Vector2Int loc in moveLocations)
        {
            ...
            if (FLGameManager.instance.PieceAtGrid(loc))
            {
    ...
    private void ExitState()
    {
        ...
        FLGameManager.instance.NextPlayer();
        selector.EnterState();
    }
