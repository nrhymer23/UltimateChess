...
public class FLMoveSelectorwAI : MonoBehaviour
{
    //Overlays
    ...
    {   
        ...
        if (Physics.Raycast(ray, out hit))
        {
            ...
            if (Input.GetMouseButtonDown(0))
            {
                ...
                if (FLGameManagerwAI.instance.PieceAtGrid(gridPoint) == null && !FLGameManagerwAI.instance.PieceHasAttacked(movingPiece))
                {

                    //game manager moves piece
                    FLGameManagerwAI.instance.Move(movingPiece, gridPoint);
                    FLGameManagerwAI.instance.CountAction();
                }
                else if (!FLGameManagerwAI.instance.PieceHasAttacked(movingPiece))
                {
                    if (FLGameManagerwAI.instance.RollToCapture(movingPiece, gridPoint))
                    {
                        //if valid move but piece there capture piece
                        FLGameManagerwAI.instance.CapturePieceAt(gridPoint);
    ...
    private void CancelMove()
    {
        ...
        FLGameManagerwAI.instance.DeselectPiece(movingPiece);
        //get tileSelector and turn it on
        FLTileSelectorwAI selector = GetComponent<FLTileSelectorwAI>();
        selector.EnterState();
    }
    ...
    public void EnterState(GameObject piece)
    {
        ...
        moveLocations = FLGameManagerwAI.instance.MovesForPiece(movingPiece);
        ...
        foreach (Vector2Int loc in moveLocations)
        {
            ...
            if (FLGameManagerwAI.instance.PieceAtGrid(loc))
            {
    ...
    private void ExitState()
    {
        ...
        FLGameManagerwAI.instance.NextPlayer();
        selector.EnterState();
    }
