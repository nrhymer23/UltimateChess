...
public class FLQueenwAI : FLPiece
{
    public override List<Vector2Int> MoveLocations(Vector2Int gridPoint)
    {
        List<Vector2Int> locations = new List<Vector2Int>();
        //gets which direction is forwards by player 
        int forwardDirection = FLGameManagerwAI.instance.currentPlayer.forward;
        ...
        foreach (Vector2Int dir in RookDirections)
        {
            for (int i = 1; i <= 5; i++)
            {
                ...
                if (FLGameManagerwAI.instance.PieceAtGrid(newRooklocation))
                {
                    break;
                }
            }
        ...
        foreach (Vector2Int dir in BishopDirections)
        {
            for (int i = 1; i <= 5; i++)
            {
                ...
                if (FLGameManagerwAI.instance.PieceAtGrid(newBishoplocation))
                {
                    break;
                }
            }
